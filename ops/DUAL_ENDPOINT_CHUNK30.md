...
The loss and its scalar weighting are unchanged from the existing paper-loss
run. Sigma-weighted EDM MSE covers every unconditioned element. Policy samples
are 75% (slots 0-4 clean; 5-12 denoised), while state-prediction samples are
25% (slots 0-5 clean; 6-12 denoised). Thus all six visual targets are included
without adding an image-loss weight.
